...
type OfferRow = { id:string; supplier_id:string; price:number; specs:unknown; warranty_months:number; delivery_days:number; product_name:string; model:string|null; image_url:string|null; benefits:string|null };
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: request, error: e1 }, { data: offers, error: e2 }] = await Promise.all([
        supabase.from("purchase_requests").select("id,title,category,budget_min,budget_max,specs,warranty_preference,delivery_preference,brands,status").eq("id", id).single(),
        supabase.from("offers").select("id,supplier_id,price,specs,warranty_months,delivery_days,product_name,model,image_url,benefits").eq("request_id", id),
      ]);
